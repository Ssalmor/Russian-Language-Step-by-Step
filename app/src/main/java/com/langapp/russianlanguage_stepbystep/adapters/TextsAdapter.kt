...
class TextsAdapter(private var data: ArrayList<ImageAndTextModel>) : RecyclerView.Adapter<TextsAdapter.ItemHolder>() {

    private var mediaPlayer: MediaPlayer? = null

    class ItemHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var icon: ImageView = itemView.findViewById(R.id.textsItemIv)
        var text: TextView = itemView.findViewById(R.id.textsItemTv)
        var card: CardView = itemView.findViewById(R.id.cardView)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemHolder {
        val itemHolder = LayoutInflater.from(parent.context)
            .inflate(R.layout.texts_item, parent, false)
